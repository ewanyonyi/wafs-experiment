...
#         true_positives = sum(1 for r in target_attack_results if r["status_code"] in DETECTION_CODES)
#         false_negatives = total_attacks - true_positives

#         false_positives = sum(1 for r in target_benign_results if r["status_code"] in DETECTION_CODES)
#         true_negatives = total_benign - false_positives

#         # Convert to ms
#         avg_attack_rt = (sum(r.get("response_time_seconds", 0) for r in target_attack_results) / total_attacks) * 1000 if total_attacks else 0
#         avg_benign_rt = (sum(r.get("response_time_seconds", 0) for r in target_benign_results) / total_benign) * 1000 if total_benign else 0
...
#     melted = df.melt(
#         id_vars="WAF", 
#         value_vars=["Detection Rate (TPR)", "False Negative Rate", "False Positive Rate", "Benign Accuracy"], 
#         var_name="Metric", 
#         value_name="Value"
#     )

#     plt.figure(figsize=(14, 7))
#     barplot = sns.barplot(data=melted, x="WAF", y="Value", hue="Metric", palette="Set2")

#     for container in barplot.containers:
#         labels = [f"{bar.get_height():.1f}%" if bar.get_height() > 1 else "" for bar in container]
#         barplot.bar_label(container, labels=labels, label_type='edge', padding=6, fontsize=9)

#     plt.title("WAF Detection Performance", fontsize=18, pad=20)
...
#     for container in barplot.containers:
#         barplot.bar_label(container, fmt="%.0f ms", label_type="edge", padding=3, fontsize=10)
...
"""
WAF Analysis Script (Enhanced Version)
...
- waf_detection_chart.png
- waf_response_time_chart.png
- waf_scatter_chart.png
- waf_analysis_results.csv
"""
...
def analyze_waf_results(attack_data, benign_data):
    ...
    df = pd.DataFrame(waf_stats).T.reset_index().rename(columns={"index": "WAF"})
    return df.sort_values(by="Detection Rate (TPR)", ascending=False)
...
def plot_detection_scatter(df):
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=df, x="False Positive Rate", y="Detection Rate (TPR)", hue="WAF", s=120, palette="dark")
    plt.title("Detection Rate vs. False Positive Rate", fontsize=16)
    plt.xlabel("False Positive Rate (%)")
    plt.ylabel("Detection Rate (TPR) (%)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("waf_scatter_chart.png")
    print("📈 Scatter chart saved as 'waf_scatter_chart.png'")
...
def main(attack_file, benign_file):
    ...
    # Save CSV and print Markdown version
    stats_df.to_csv("waf_analysis_results.csv", index=False)
    print("📄 Stats saved as 'waf_analysis_results.csv'\n")
    ...
    plot_detection_chart(stats_df)
    plot_response_time_chart(stats_df)
    plot_detection_scatter(stats_df)
